...
class SpriteSheet:
    ...
    def draw_frame_iso(self, pos: Tuple[int, int], i, fliph: bool) -> DrawCall:
        r = self.rect(int(i))
        if fliph:
            r.width = -r.width
        return (self.texture, r, rl.Rectangle(*xy_to_iso((pos[0] % VIEW_SIZE, pos[1] % VIEW_SIZE)), self.width, self.texture.height),
                (self.width / 2 - BLOCK_WIDTH / 2, self.texture.height), 0, rl.WHITE)

    def draw_frame_iso_tile(self, pos: Vector2i, i) -> DrawCall:
        r = self.rect(int(i))
        return (self.texture, r, rl.Rectangle(*xy_to_iso((pos[0] % VIEW_SIZE, pos[1] % VIEW_SIZE)), self.width, self.texture.height),
                (self.width / 2 - BLOCK_WIDTH / 2, self.texture.height - BLOCK_HEIGHT / 2), 0, rl.WHITE)
...
maparr = (np.array([tile_type.get((map_pixels[i].r, map_pixels[i].g, map_pixels[i].b, map_pixels[i].a), TileType.NONE)
          for i in range(map_img.width * map_img.height)], dtype='int8').reshape((map_img.height, map_img.width)))
...
def walk_path(maparr, hero, goal):
    ...
    last_pos = hero.pos
    while path:
        draw_calls.append(hero_sprite.draw_frame_iso(hero.pos, (t * 8) % 4, xy_to_iso(hero.pos).x - xy_to_iso(last_pos).x > 0))
        yield
    ...
    for y in range(VIEW_SIZE):
        ...
        for x in range(VIEW_SIZE):
            ...
            if maparr[gy][gx] == TileType.FIRE:
                draw_calls.append(fire_tile.draw_frame_iso_tile((x, y), int((rl.get_time() % 0.5) / 0.1)))
